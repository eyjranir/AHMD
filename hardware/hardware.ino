...
char host[] = "192.168.100.50"; //Địa chỉ IP dịch vụ, hãy thay đổi nó theo địa chỉ IP Socket server của bạn.
...
void loop()
{
  ...
  if (millis() - tsLastReport > REPORTING_PERIOD_MS)
  {
    ...
    GuiDuLieu(nhiptim, nhietdo, oxy, Co, lng, lat);
  }
...
void GuiDuLieu(float nhiptim, float nhietdo, float oxy, int Co, float lng, float lat)
{
  if (nhiptim > 60)
  ...
  if (lng != 0)
  {
    client.send("LNG", "message", String(lng, 6));
